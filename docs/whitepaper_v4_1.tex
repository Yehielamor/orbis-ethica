...
\rhead{\small \textsc{Whitepaper V4.1}}
\cfoot{\thepage}
...
\title{
    ...
    \hrule height 2pt
    \vspace{2cm}
    \large \textbf{Whitepaper Version 4.1}
}
...
Where:
\begin{itemize}
    \item $\alpha, \beta, \gamma, \delta$ are weights determined by the Ethical DAO during Recalibration Epochs.
    \item $U(a)$ and $L(a)$ measure positive contributions.
    \item $F_{penalty}(a)$ penalizes unfair distributions (higher when inequality is severe).
    \item $Risk(a)$ captures uncertainty, irreversibility, and potential for catastrophic harm.
\end{itemize}
...
\section{Current Implementation Status (V4.1)}

\subsection{The Clear Layer (Knowledge Gateway)}
Building on the architecture described in Section 3.1, V4.1 introduces the fully implemented **Knowledge Gateway**:
\begin{itemize}
    \item \textbf{Ingestion API}: A dedicated endpoint (\texttt{/api/knowledge/ingest}) for submitting raw data.
    \item \textbf{Verification Pipeline}:
    \begin{itemize}
        \item \textbf{Cryptographic Check}: Verifies the signature of the data provider.
        \item \textbf{Source Whitelisting}: Checks if the source ID is in the trusted registry (e.g., ``WHO\_Secure\_Feed'').
        \item \textbf{Purity Scoring}: Assigns a confidence score to the data before it enters the Memory Graph.
    \end{itemize}
    \item \textbf{UI Integration}: A dedicated ``Knowledge'' tab allows users to submit and verify data in real-time.
\end{itemize}

\subsection{Advanced Simulation \& Deployment}
\begin{itemize}
    \item \textbf{Dockerization}: Full containerization of Backend (FastAPI) and Frontend (Nginx) for reproducible deployments.
    \item \textbf{Adversarial Testing}: Automated scenarios (\texttt{scenarios/adversarial.py}) that simulate ``Sybil Attacks'' and ``Data Poisoning'' to verify the Burn Protocol's response.
    \item \textbf{Stress Testing}: High-load simulations (\texttt{scenarios/stress\_test.py}) to ensure system stability under rapid deliberation rounds.
    \item \textbf{Rate Limiting}: API protection using \texttt{slowapi} to prevent abuse.
\end{itemize}

\subsection{Phase VIII: P2P Decentralization}
The system has recently evolved to a fully peer-to-peer architecture:
\begin{itemize}
    \item \textbf{Node Discovery}: Implementing a distributed hash table (DHT) or similar mechanism for peer finding.
    \item \textbf{Gossip Protocol}: Efficient broadcasting of blocks and votes across the network.
    \item \textbf{Consensus Upgrade}: Moving from local consensus to a distributed consensus mechanism (e.g., Proof of Stake or Proof of Authority).
\end{itemize}

\section{Roadmap}
\begin{itemize}
    \item \textbf{Phase I: Proof of Concept (Months 1-4)}: Minimal Ethical Core, 3 entities, local consensus. (Completed)
    \item \textbf{Phase II: Open Dialogue Network (Months 5-9)}: All 6 entities, Purification Gateway, Testnet. (Completed)
    \item \textbf{Phase III: Governance & Burn (Months 10-15)}: Ethical DAO launch, Burn Protocol, Global Assembly. (In Progress)
    \item \textbf{Phase IV: Federation & Scale (Months 16-24)}: API for external AI systems, Mainnet launch.
\end{itemize}
